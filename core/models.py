...
class ABAW3Model(LightningModule):
    ...
    def __init__(self):
        ...
        self.drop = nn.Dropout(0.5)
            ...
            #self.fc1 = nn.Linear(888*3,self.num_outputs)
            #self.feed1 = FeedForward(dim = 888, hidden_dim = 512)
            #self.drop1 = nn.Dropout(0.5)
        #elif self.model_name == "only_att":
            #self.att2 = Attention(dim = 888, heads = 2, dim_head = 64, dropout = 0.)
            #self.fc2 = nn.Linear(888*3,self.num_outputs)
            #self.feed2 = FeedForward(dim = 888, hidden_dim = 512)
            #self.drop2 = nn.Dropout(0.5)
        #elif self.model_name == "only_trans":
            #self.trans3 = Transformer(dim = 888, depth = 1, heads = 2, dim_head = 64, mlp_dim = 512, dropout = 0.)
            #self.fc3 = nn.Linear(888*3,self.num_outputs)
            #self.feed3 = FeedForward(dim = 888, hidden_dim = 512)
